...
| `docs` | `--wss-docs` `--wss-diagram` | Writes and maintains a project's long-form documentation site, every claim anchored to a real source path — including per-workflow flow pages, a Mermaid diagram plus stages citing the behaviour record and the code. `--wss-diagram` is the ad-hoc entry: one diagram, drawn under the style guide's three rules, landed as an annex page |
...
| `tools` | `--wss-tools` | Keeps this catalog current, hands it to `--wss-docs` where a site exists, deletes stale claims from skill and agent files, and runs the prose prune over the same set |
