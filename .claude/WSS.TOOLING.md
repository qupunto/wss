...
`--wss-health-check` invokes it after any edit that restructures a tooling file, and
`wss-tools-inventory.sh` runs before either renders, so the rows below describe
the tree the run ends with rather than the one it started from — which is what
...
| `--wss-health-check` | `wss-survey` for the file-sharded read fan-out, `wss-duplication.sh`, `wss-mechanical-gauntlet.sh`, the owner of each record finding through `WSS.FINDING-DISPATCH.md`, `--wss-todo`, `--wss-log`, `--wss-catalog`, `wss-tools-inventory.sh`, `git-writer`, `sweep-tracker` — plus the project's own publish assembly under `--publish` **where it has one — this suite's assembler does not travel, so an adopted tree has none and that mode declines** — and, `--deep`'s TODO resort once taken, the manifest's `WSS.agents.audit` fan-out, `--wss-plan`, `--wss-release`, `--wss-wrap`, `audit-writer`, `handoff-writer` | one skill, four mutually exclusive modes, where `--wss-check`, `--wss-full-check`, `--wss-stocktake` / `--wss-full-stocktake` and `--wss-tidy` used to be four flags on five skills — the decision log's `2026-08-19 (eighty-second)` entry. Reads the mode's scope, runs the mechanical floor, dispatches and applies record and tooling findings itself rather than through a separate skill, regenerates the catalog and the inventory after any restructuring, commits by concept, and stamps a checkpoint only for a scope that came back healthy |
| `triage` | `WSS.INBOX-TRIAGE.md`'s method, the finding's own owner once classified | works the suite's own defect inbox — the machine-local file and the upstream repository — deciding which entries reproduce and which are stale. **User-invocable only**: no `--wss-health-check` mode runs it, which is why it stayed a skill of its own rather than becoming a step |
...
| `health-check` | `--wss-health-check` | Asks whether the project is healthy — the mechanical floor, then every declared record and tooling file, read for drift and staleness. **One skill, four mutually exclusive modes**: `--shallow` reads and reports and applies nothing; the bare run adds dispatch-and-apply; `--deep` ignores every checkpoint and ask-gates a TODO resort; `--publish` narrows to the shipping set with the publish gates in its floor. It stamps a checkpoint **only for a healthy scope** — no findings, or every finding fixed or dispatched — so unhealthy files stay inside the next run's slice; the definition lives once, in `WSS.SWEEP-CHECKPOINT.md`'s rule 5. Reports the inbox's open count at hand-off and never triages it |
| `triage` | `--wss-triage` | Works the suite's own defect inbox — which filed defects reproduce, which went stale under a moving tree, which were never true. A thin route to `WSS.INBOX-TRIAGE.md`, which holds the method. **User-invocable only**: no `health-check` mode runs it, which is the whole reason it is a skill of its own rather than a step |
...
| `audit` | `/wss:audit` only — no flag, model invocation blocked | Runs an independent audit pass over the suite and files the frozen report into an `audits/` directory beside the index `WSS.record.audits` names, working from the cumulative rubric in [`WSS.AUDIT-PASS.md`](../wss/tests/WSS.AUDIT-PASS.md) — findings settled by execution rather than by reading, verdicts anchored, proposals in an annex. Refuses to run in a session that prepared the tree it would be judging. **Commits, never pushes, and fixes nothing** |
<!-- wss:region-end -->
...
`--wss-start` and `--wss-health-check` read it, `--wss-wrap` counts it. None of
