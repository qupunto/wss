...
2. **One writer.** Does exactly one procedure write this file? Two writers each
   ...
   [`WSS.OWNERSHIP.md`](../workflow/WSS.OWNERSHIP.md)'s sole-writer column, and its
...
5. **Write the form that cannot go false.** Is this a rule, or a state? Counts,
   ...
   **And a cardinality lives with its items.** The predicate stops at the file
   ...
   [`WSS.SWEEP-CHECKPOINT.md`](../workflow/WSS.SWEEP-CHECKPOINT.md)'s four-rules heading
   was restated verbatim as a heading in
   [`WSS.AUDIT-COVERAGE.md`](../workflow/WSS.AUDIT-COVERAGE.md), which holds none of them —
   that heading now names the rules without counting them, and only the file
   holding the set still says how big it is. The other half of the failure is
   still live: the set is cited by number from files that cannot see it.
...
8. **Freshness is stamped, not remembered.** Can *when was this last verified* be
   ...
   [`WSS.SWEEP-CHECKPOINT.md`](../workflow/WSS.SWEEP-CHECKPOINT.md)'s freshness-only
...
10. **Citations survive renames.** Does this reference something whose name can
    ...
    [`WSS.RECORD-CONTRACT.md`](../workflow/WSS.RECORD-CONTRACT.md) forbids rewriting the
...
12. **The write mode matches the content.** Is this history, or state? A log
    ...
    Proven: [`WSS.RECORD-CONTRACT.md`](../workflow/WSS.RECORD-CONTRACT.md)'s two write
...
[`WSS.LANE-CONTRACT.md`](../workflow/WSS.LANE-CONTRACT.md) allows one marker and no
ladder.
