...
| a commit that implements what a TODO list item describes, or that clears the blocker one is parked on | `WSS.record.todo` — the lane's, where a `.claude/WSS.LANE` selector resolves one. Where it names a provider there is no file to diff; the paragraph below applies |
...
- **A roadmap carries a version number or a completion mark.** Neither belongs
  ...
  when a sweep reads the file anyway.
...
- **History is not staleness.** Never report a "fix" for an entry in
  ...
  [`WSS.RECORD-CONTRACT.md`](../workflow/WSS.RECORD-CONTRACT.md)'s rule 4. Stated here rather than
