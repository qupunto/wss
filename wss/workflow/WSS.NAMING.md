...
**The borrowed convention travels with the file; the borrowed location does
not.** `README`, `CHANGELOG` and `LICENSE` still sit at the repository root —
they are the ecosystem's, not the suite's (see "Ownership is authorship, not
usage" below). No WSS-owned file does any more: `wss-doctor.sh` sits at
`wss/tests/wss-doctor.sh`, `WSS.TODO.md` at `wss/records/WSS.TODO.md`, and
every other caps or `wss-` file the suite creates lives somewhere under
`skills/`, `agents/`, `commands/`, `hooks/`, or `wss/`. The form in the table
above is about ROLE, never about which directory a file sits in.
...
- **Everything under `commands/`, and it is the subtle one** — a wrapper's
  filename *is* its invocation, so the harness resolves it by name, and the
  name it must carry depends on the install form. A checkout keeps the prefix
  (`commands/log.md` → `/wss:log`); a plugin install namespaces the wrapper
  and the published tree therefore ships `commands/log.md` → `/wss:log`. **The
  prefix is the namespace's job in one form and the filename's in the other**,
  so no single spelling is correct for both and this convention cannot be
  asserted over them. What pins a wrapper's name instead is stricter than this
  contract: its body must fire the flag its own name promises, and that flag
  must exist in the hook. `wss-doctor.sh`'s "Command wrappers" section is where
  that lives, and it knows both forms.
