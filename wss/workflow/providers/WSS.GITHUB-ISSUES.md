...
**A TODO list provider.** Where a project declares one, `WSS.record.todo` is not a
...
`repo` is required. `label` is optional; without it the TODO list is **every open
...
A team that already lives in Issues cannot adopt a workflow whose TODO list is a
...
**Two things a file TODO list carries in its shape do not survive the move, and
both are load-bearing for `--wss-start`.**

**Order is not priority here.** A markdown TODO list is severity- and
...
edit the title to say DONE — the markdown rule is that a TODO list is
...
rest, so a TODO list of 45 silently becomes 30 — the failure class this whole
...
- a **count read after closing** — which is exactly what `--wss-wrap` does, landing
  ...
  many items just left the TODO list.
...
read silently sees an empty TODO list.
...
TODO list items needs a limit above 400 to see all 20. The same ceiling warning
applies, harder.
...
can be behind, which is the only race a session can do anything about. The rule
...
- `gh` absent → say the TODO list could not be reached and name the item that was
...
`--wss-check` and `--wss-stocktake` read the TODO list the same way, through the list
...
is not visible in the git history at all, so an issue TODO list is always read in
