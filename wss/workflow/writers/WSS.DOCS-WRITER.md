...
> **A procedure, not a skill** — see [`WSS.WRITERS.md`](WSS.WRITERS.md), whose [read-inheritance rule](WSS.WRITERS.md#read-inheritance) this follows. Sole writer of the documentation site, per [`WSS.OWNERSHIP.md`](../WSS.OWNERSHIP.md).
...
- **Not `WSS.record.tooling.catalog`.** The catalog is `--wss-catalog`'s, and the
  annex page derived from it is yours. The derivation runs one way.
- **Not `README.md`'s body**, where the manifest maps it as
  `WSS.record.reference`. Step 3's wire-up adds the new page's navigation link
  there and touches nothing else; the surrounding prose is
  [`WSS.REFERENCE-WRITER.md`](WSS.REFERENCE-WRITER.md)'s. Two procedures write
  this one file, so the boundary is stated on both sides rather than inferred.
...
- **A diagram handed over by a caller is re-rendered, not copied.** `--wss-catalog`
...
[`WSS.DOCS-AUDIT.md`](../../tests/WSS.DOCS-AUDIT.md) — its mechanics sections and
