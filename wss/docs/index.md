...
coding rather than a coder*: they keep a project's TODO list, decision log,
...
| [domain/WSS.WORKFLOWS.md](domain/WSS.WORKFLOWS.md) | End-to-end journeys through the suite. Currently one: a batch from `--wss-start` to `--wss-wrap`, its ten stages, its three human gates, and the three places the obvious narration is wrong |

## Annex

| File | Description |
|---|---|
| [annex/WSS.LANE-SYNCHING.md](annex/WSS.LANE-SYNCHING.md) | How work crosses between lanes without any lane writing another's records — the transfer queue, the `[critical → why]` marker, and `/wss:lane-record-sync` |
