...
must agree with the matrix in `wss/workflow/WSS.OWNERSHIP.md` — `wss-doctor.sh`
...
For this flow the grant is the row `wss/workflow/WSS.OWNERSHIP.md::build`:
**commit, not push.**
...
both; `wss/workflow/WSS.RECORD-CONTRACT.md` rule 3.
...
because a roadmap block is a paragraph and a shard needs a file list.

### 5 — Partition by file sets, counting reads as well as writes

Shards run concurrently in one working tree, so the unit of parallelism is the
file set rather than the task.

`wss/workflow/WSS.FAN-OUT.md`, which `skills/start/SKILL.md::Phase 3 —
Partition into shards that cannot collide` reads rather than restates.
The non-obvious half is the read set. Two shards where one *reads* what another
...
admits at most one shard in the whole batch, `WSS.lanes.serialize` covers shared
machinery, `WSS.lanes.generated` belongs to nobody —
`wss/workflow/WSS.MANIFEST.md`. Every record file is off-limits to every shard.

### 6 — The wave runs, and no shard touches a record

All shards of a wave dispatched in one message so they are genuinely concurrent,
each with its write list, its read list, the decision entries that bear on it,
and whether it may run the test suite at all.

`wss/workflow/WSS.FAN-OUT.md`, which `skills/start/SKILL.md::Phase 4 — Run the
wave` reads rather than restates. Where the project declares
a consent-gated suite (`WSS.commands.testConsentEnv`,
`wss/workflow/WSS.MANIFEST.md`), a shard cannot run it — not the whole suite, not one
...
run were the sanctioned alternative, and a shard discovering otherwise has
already spent the attempt.

### 7 — Integrate, and commit before testing

Read every shard's diff, regenerate anything generated, typecheck, then commit
each shard separately.

`skills/start/SKILL.md::Phase 5 — Integrate and verify, once`, and
`wss/workflow/writers/WSS.GIT-WRITER.md` for the commits. Committing before the
...
Typecheck sits here and nowhere else because cross-shard type breakage is the
one failure no individual shard can see — each shard's own transcript is green.

A shard that dies without reporting is treated as **suspect rather than
incomplete**: shards are told to prove a guard by breaking the code on purpose,
...
`wss/workflow/WSS.SWEEP-CHECKPOINT.md` for the checkpoint's fields. The stamp goes
through `wss/workflow/writers/WSS.SWEEP-TRACKER.md` and is written **only** after a
...
What shipped leaves the TODO list, non-obvious calls reach the decision log,
...
against the matrix in `wss/workflow/WSS.OWNERSHIP.md`.
...
Phase 6 refreshes the handoff through `wss/workflow/writers/WSS.HANDOFF-WRITER.md`
and stops. It does not call `--wss-wrap`.

`skills/start/SKILL.md::What this skill does not do`, and
`wss/workflow/WSS.OWNERSHIP.md::hand off` for the contrasting grant. An invoked
