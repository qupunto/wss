...
A condensed copy of the canon table in `wss/workflow/WSS.LANE-CONTRACT.md`,
sanctioned under `wss/workflow/WSS.RECORD-CONTRACT.md`'s exception 2: this
table's cells are free to shorten, but every row label must still be one the
contract carries.
...
`--wss-start` does with it are `wss/workflow/WSS.LANE-CONTRACT.md`'s
...
**Derived from [skills/lane-record-sync/references/WSS.STEP3-DEPENDENCIES.md](../../../skills/lane-record-sync/references/WSS.STEP3-DEPENDENCIES.md)'s
own copy of this table — regenerate with `bash wss/scripts/wss-gen-lane-rulings.sh` from
the repo root; the block below is not hand-edited.**

<!-- BEGIN GENERATED by wss-gen-lane-rulings.sh — do not edit by hand -->
