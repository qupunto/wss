...
│
├── wss/                   everything the suite owns that is not a skill, agent or hook
│   ├── scripts/                      the operational set — none of these live at the root
...
│   │   ├── wss-mechanical-gauntlet.sh runs doctor, typecheck, the full suite and CI in one call
│   │   ├── wss-append-only.sh         fails any change deleting a line from a record tagged `log`
│   │   ├── wss-commit-provenance.sh   declares in the commit message which records it writes, and under whose authority
│   │   ├── wss-sweep-stamp.sh         writes one sweep-checkpoint entry; baselines are computed, never typed
│   │   ├── wss-tools-inventory.sh     walks the tooling surface and writes `.claude/WSS.TOOLS.json`
│   │   ├── wss-docs-audit.sh          the mechanical half of the docs audit, as named subcommands
│   │   ├── wss-duplication.sh         reports repeated prose across the tooling surface
│   │   ├── wss-gen-cadence-flags.sh   regenerates the cadence table README.md carries
│   │   └── wss-gen-lane-rulings.sh    regenerates the lane-ruling table from the decision log
│   ├── tests/                        the checks, and the contract tests whose breakage is silent
│   │   ├── wss-doctor.sh              read-only health check — see "Verifying" below
│   │   ├── wss-hook-contract.sh       the hook contract suite
│   │   └── wss-append-only-test.sh    the append-only guard's own cases
│   ├── workflow/                     the contracts every skill links to instead of copying
│   ├── records/
│   │   ├── WSS.TODO.md            WSS.record.todo
│   │   ├── WSS.BACKLOG.md         WSS.record.backlog — findings nobody scheduled; no checkboxes
...
│   │   ├── WSS.OPEN-DECISIONS.md  WSS.record.openDecisions
│   │   ├── WSS.HANDOFF.md         WSS.record.handoff
│   │   └── WSS.HAZARDS.md         the standing hazards, split out of the handoff
│   ├── logs/
│   │   ├── WSS.CHANGELOG.md       WSS.record.changelog — the engineering log
│   │   ├── WSS.DECISIONS.md       the decision log
│   │   ├── WSS.AUDITS.md          WSS.record.audits, one row per report
│   │   ├── WSS.STOCKTAKE.md       WSS.record.stocktake — a log, and filed as one
│   │   └── audits/                frozen agent audit reports; index in WSS.AUDITS.md
│   ├── generated/
│   │   └── WSS.DECISIONS-INDEX.md generated index of decisions
│   ├── flags/                     untracked machine-local flags
...
`wss/workflow/` sits outside `skills/` on purpose. A directory under `skills/` with
...
the conventional names in `wss/workflow/WSS.MANIFEST.md` and skip what they cannot
...
| `skills/*/references/*.md` | only when the owning skill's `SKILL.md` cites one | the router shape: a short `SKILL.md` whose body is a table of steps or modes, each behind its own reference. Mode-exclusive skills genuinely save; sequential ones mainly stop being charged at session start |
...
record procedures left `skills/` for `wss/workflow/writers/` and stopped being
...
be re-anchored across plugin boundaries. [`wss/logs/WSS.DECISIONS.md`](../logs/WSS.DECISIONS.md)
...
named the contracts as `~/.claude/wss/workflow/*.md`, which resolves to nothing under
...
- **`SessionStart` → `hooks/wss-session-check.sh`.** Silent when there is nothing worth a
  ...
  decisions unchanged for 25 commits, or a TODO list or roadmap unchanged for 80 —
  ...
  this block; and never for a TODO list that names a provider rather than a file,
...
```bash
wss/tests/wss-doctor.sh                    # read-only; prints every check except quiet notes (--notes shows those too)
bash wss/tests/wss-hook-contract.sh    # the hook's contract
```
...
- `hooks/wss-shorthand-flags.sh` — the `FLAGS` array is the set of tokens the hook will
  ...
  readers in `README.md`, `wss/workflow/WSS.OWNERSHIP.md`, `.claude/WSS.TOOLING.md` and
  [the annex](annex/WSS.CLAUDE-TOOLING.md), so treat none of those as the source.
  The grants are restated in exactly one other place — `wss/workflow/WSS.OWNERSHIP.md` —
...
- `wss/workflow/WSS.OWNERSHIP.md` — the authority on who may write what. It settles any
  disagreement between two skills.
- `wss/workflow/WSS.RECORD-CONTRACT.md` — what each record file holds and must not hold.
- `wss/workflow/WSS.MANIFEST.md` — which keys a project's `.claude/WSS.WORKFLOW.json` may
  set, and what each falls back to when absent.
- `wss/workflow/WSS.LANE-CONTRACT.md` — everything the suite holds about worktree
