...
holds is [`WSS.RECORD-CONTRACT.md`](../../wss/workflow/WSS.RECORD-CONTRACT.md); the checkpoint
format is [`WSS.SWEEP-CHECKPOINT.md`](../../wss/workflow/WSS.SWEEP-CHECKPOINT.md).
...
[`WSS.LANE-CONTRACT.md`](../../wss/workflow/WSS.LANE-CONTRACT.md)'s resolution rule. Other lanes'
...
| `WSS.record.tooling.catalog` | skills and agents that no longer exist |
| `WSS.record.tooling.sources` | mutable claims that should be deleted, prose that changes nothing |
| `WSS.record.audits` | its opening prose only — a header claim contradicted by reality, a pointer that no longer resolves; the per-report rows are frozen history, per [`WSS.RECORD-CONTRACT.md`](../../wss/workflow/WSS.RECORD-CONTRACT.md)'s header split. Findings dispatch to `audit-writer` |
...
TODO list is a set of open issues rather than a file, per
...
checkpoints by definition. Where `gh` cannot reach it, say the TODO list was not
...
nets and the TODO list rebuild are `--wss-full-stocktake`'s, and correctness, security
...
- **Records** — [`wss/tests/WSS.RECORD-DRIFT.md`](../../wss/tests/WSS.RECORD-DRIFT.md),
  at full scope, with every incremental narrowing ignored.
- **Docs site** — [`wss/tests/WSS.DOCS-AUDIT.md`](../../wss/tests/WSS.DOCS-AUDIT.md),
  every section, over every page — the incremental narrowing lives in `--wss-docs`
  and is simply not applied here.
- **Tooling** — [`wss/tests/WSS.TOOLING-CLAIMS.md`](../../wss/tests/WSS.TOOLING-CLAIMS.md),
  over every file in `WSS.record.tooling.sources`.

Hand each reader the file, not a skill.

**Each reader is `wss-survey`**, not a general-purpose subagent — same brief,
same verdict, and materially cheaper before it reads anything, because a grant
of `Tools: *` ships every write and web schema this rung is forbidden to use.
`agents/wss-survey.md` already carries the report-don't-write contract, so a
brief here names only the method file, the read set and the drift shape.
...
fix, and they do not write. Each reader matches
[the dispatch ladder's](../../wss/workflow/WSS.DISPATCH-LADDER.md) Survey rung — it
writes no files, its file list is named above rather than chosen by the agent,
and its verdict is the drift shape its brief pins — at the bottom tier, low
effort. This is every whole-record read this skill does on its own account: the
defect-inbox triage below and the pre-report verification are dispositions —
deciding what a finding means, routing it, closing it — so they stay Keep and
inline regardless of size, per the ladder's first key.

### 4. Triage the defect inbox

**The method is
[`wss/tests/WSS.INBOX-TRIAGE.md`](../../wss/tests/WSS.INBOX-TRIAGE.md)** —
what an entry is, the staleness probe, the re-verification, the confirmed /
stale / did-not-reproduce split, what closing requires, and the rule that each
half of the inbox is reported separately.

What the method leaves to this runner:

- **Whether it may run at all. In scope only when this project *is* the
  configuration directory.** From anywhere else, filing is the whole action a
  session may take —
  [`WSS.OWNERSHIP.md`](../../wss/workflow/WSS.OWNERSHIP.md) is the authority, and
  triaging another repository's inbox from this one's session is the same
  mistake one indirection along. Say in one line that it was skipped and why.
- **Where the two halves live here**: `WSS.BUG-REPORTS.md` in the config
  directory, where the entries to triage are the `[open]` ones **below the
  append marker** — and `gh issue list -R qupunto/wss --state open` for the
  upstream half, where the issue body is the entry. Where `gh` cannot reach the
  repository, the upstream half is **not checked**.
- **Where a confirmed finding goes**: with the rest of this run's findings, in
  step 6 — it is a finding like any other, and its owner is looked up the same
  way.
- **The act of closing.** `[open]` becomes `[closed]` and the commit says what
  landed; an entry that did not reproduce is deleted, with the commit naming
  what was claimed and why it is false. Upstream, that is
  `gh issue close <n> --comment "<what landed, or why it does not reproduce>"`,
  and the method's published-not-merely-landed condition binds.

### 5. Verify before anything reaches the user

Re-check each finding against the cited file and line
([`WSS.OWNERSHIP.md`](../../wss/workflow/WSS.OWNERSHIP.md#the-inspector-writes-nothing)), and
...
[`WSS.OWNERSHIP.md`](../../wss/workflow/WSS.OWNERSHIP.md). The owner re-verifies and writes
...
**Run `tidy`'s prune job after the dispatch, not before:**
...
It reports and dispatches; it does not cut. `--wss-tidy` makes the cut after its own
second look.

Then hand `WSS.record.tooling.catalog` to `--wss-catalog`, which owns it: add, edit or
...
[`WSS.SWEEP-CHECKPOINT.md`](../../wss/workflow/WSS.SWEEP-CHECKPOINT.md).
