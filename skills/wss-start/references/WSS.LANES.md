...
- **Carry `[critical → why]` across unchanged**, and never add one. A lane may
  ...
  means an entry arrives marked from one of `wss-lane-record-sync`'s two gates —
...
came through `wss-lane-record-sync`'s approval gate was already ruled on by the
...
assumption becomes the project's by default. `wss-lane-record-sync` is the only
