...
1. **Pin the tree.** `git rev-parse --short HEAD`, `git status --porcelain`, and
   ...
   **A lane worktree syncs forward before anything reads a WSS.record.** Where a
   `.claude/WSS.LANE` selector is present and the worktree's branch is behind
   `WSS.branch.integration`, `git fetch` and `git merge --ff-only` it up to date
   ...
   **Drain the lane's transfer queue, after the sync-forward and before
   anything below reads a WSS.record.** `WSS.lanes.named.<lane>.transfer` is this lane's
   ...
   here only once that lane has landed on `WSS.branch.integration` and step 1 has
   ...
   - **Move it into the record its `[target]` names** — `todo`,
     ...
     same as any other: `--wss-todo` for the first two, `--wss-plan` for a
     roadmap goal.
   ...
   - **Carry `[critical → why]` across unchanged**, and never add one. A lane
     ...
     `wss-lane-record-sync`'s two gates — a mediated conflict, or an *accept
     as critical* ruling.

   **Eligibility follows provenance; priority follows the marker.** An entry
   that came through `wss-lane-record-sync`'s approval gate was already ruled
   ...
   **A contradiction found mid-batch goes to `WSS.lanes.conflicts`, not into the
   ...
   `wss-lane-record-sync` is the only consumer, and it re-verifies the claim
   rather than acting on it —
   [`WSS.RECORD-CONTRACT.md`](../../workflow/WSS.RECORD-CONTRACT.md) holds the entry
