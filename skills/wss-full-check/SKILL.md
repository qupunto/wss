...
**The inbox has an upstream half: open issues on `qupunto/workflow-secretary-suite`.**
...
`gh issue list -R qupunto/workflow-secretary-suite --state open` and take each open
...
**Run `wss-tools`'s prune job after the dispatch, not before:**
