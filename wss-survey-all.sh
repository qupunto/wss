...
# repository `gh` could not read has an UNKNOWN backlog rather than a file one —
# which matters exactly here, because "the Issues provider has no candidate" is
# a conclusion drawn from backlog rows. Every unread probe is therefore carried
...
usage() {
  ...
  echo "  --no-gh      passed through: skip the backlog probe"
...
cat <<'LEGEND'
  flags  D doctor unread (not found / failed)   d doctor skipped by request
         B backlog UNKNOWN (gh could not read)  b backlog probe skipped
  ...
  about that project's backlog, including "it uses a file".
LEGEND
...
if [ "$HAVE_JQ" -eq 0 ]; then
  ...
  surface "Issues backlog provider"    '($w.record.todo | type) == "object"'
  ...
  hits=0; names=""
  for t in "${REPS[@]}"; do
    if [ -f "$t/docs/index.html" ] || [ -f "$t/docs/_sidebar.md" ] || [ -f "$t/docs/index.md" ]; then
      hits=$((hits+1)); names="$names $(basename "$t")"
    fi
  done
  if [ "$hits" -eq 0 ]; then
