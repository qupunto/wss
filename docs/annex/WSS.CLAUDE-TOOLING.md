...
| `tools` | `--wss-tools` | Keeps the catalog current, hands it to `--wss-docs` where a site exists, deletes stale claims from skill and agent files, and runs the prose prune over the same set |
...
project declares, and this repository declares none of those — so a skill that
