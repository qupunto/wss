...
# tree split records by lane, is its backlog a file or a provider, does it
...
# The doctor ships beside this script, so its own location needs no environment
# to be known — the same reason wss-doctor.sh asks where IT lives. The other two
...
DOCTOR_SH=""
for cand in "$SELF_DIR/wss-doctor.sh" "$CONFIG_DIR/wss-doctor.sh"; do
  [ -f "$cand" ] && { DOCTOR_SH="$cand"; break; }
done
if [ -z "$DOCTOR_SH" ]; then
  DOCTOR_SH=$(ls -1 "$CONFIG_DIR"/plugins/cache/*/*/*/wss-doctor.sh 2>/dev/null | tail -1)
fi
...
# index under v2 (workflow/WSS.MANIFEST.md, "record" section). Reading a legacy
...
row "backlog" "$(m '$w.record.todo | if . == null then empty elif type == "object" then "PROVIDER: \(.provider // "?") \(.repo // "") \(.label // "")" else "file: \(.)" end' "not declared (fallback WSS.TODO.md)")"
...
if [ "$GH" -eq 0 ]; then
  ...
  # at all. Collapsing the third into the second reports "file backlog" for a
  # tree whose backlog was never looked at.
  if [ -n "$gh_out" ]; then
    printf '%s\n' "$gh_out" | head -5 | sed 's/^/  /'
  else
    echo "  (no open issues — a file backlog, or an empty one)"
  fi
else
