...
> **A procedure, not a skill** — see [`WSS.WRITERS.md`](WSS.WRITERS.md). Sole writer of the sweep checkpoint, per [`WSS.OWNERSHIP.md`](../WSS.OWNERSHIP.md).
...
The caller hands over its baseline, its method, and per scope the globs it
covered and the globs it did not.
...
3. **Write the entry**, replacing any previous one for that key. The checkpoint
   ...
   swept when belongs to `WSS.record.stocktake` where a project keeps one.
4. **Ensure the file is gitignored** before the first write — the file the
   *manifest* resolved to in Resolve step 1, never a literal path, or a project
   ...
   the real one. `git check-ignore -q "$sweeps"`; if it is not ignored, add the
   rule and say so. A committed checkpoint is read as authoritative on a machine
   that never ran the sweep.
